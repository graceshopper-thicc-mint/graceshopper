...
const CartSingleItem = ({ game, adjustItemQty, removeFromCart }) => {
  ...
  return (
    <div key={game.id} id={`id${game.id}`} className="in-cart-game">
      <p className="in-cart-game-name">{game.name}</p>
      <div>
        <img className="in-cart-game-img" src={game.imageUrl} />
        <div className="in-cart-desc">
          <div>
          ...
          <button onClick={() => removeFromCart(game)}>x</button>
          </div>
          <p className="in-cart-game-price">$ {game.price}</p>
        </div>
      </div>
    </div>
  );
