...
export const addToCart = (game) => {
  ...
  return async (dispatch) => {
    if (Object.prototype.hasOwnProperty.call(localStorage, "token")) {
      ...
      if (
        invoice &&
        cartDb.map((invoiceLine) => invoiceLine.gameId).indexOf(game.id) === -1
      ) {
        ...
        let { data: invoiceLine } = await axios.get(
          `/api/users/${userId}/cart/${game.id}`
        );
  ...
  return async (dispatch) => {
    if (Object.prototype.hasOwnProperty.call(localStorage, "token")) {
      ...
      const gamesAwaiting = cartDb.map(async (invoiceLine) => {
        let { data: gameToFetch } = await axios.get(
          `/api/games/${invoiceLine.gameId}`
        );
        gameToFetch.itemQuantity = invoiceLine.itemQuantity;
        gameToFetch.price = gameToFetch.price / 100;
        return gameToFetch;
      });
      const gamesPromise = Promise.all(gamesAwaiting)
        .then((game) => {
          return game;
        })
        .catch((err) => {
